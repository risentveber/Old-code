...
int main(){
	list<Abonent> base;
	vector<list<Abonent>::iterator> selection;
	map<string, list<Abonent>::iterator> nameSearch;
	map<unsigned _int64, list<Abonent>::iterator> phoneSearch;
	string command;
	string name;
	string name_2;
	string e_mail;
	unsigned _int64 phone;
	unsigned _int64 newPhone;
	/*
	exit
	remove
	add
	...
	*/
	while (command != "exit"){
		...
		if (command == "help"){
			cout << "search <string::name> - print list of abonents with prefix name" << endl;
		...
		if (command == "add_with_e-mail" || command == "add"){ //добавление абонента с e-mail или без него
			...
			if (it == nameSearch.end()){
				...
				base.push_front(Abonent(name, phone, e_mail));
		...
		if (command == "phone"){
			...
			if(it != phoneSearch.end()){
				...
				cout << "There is no abonent with this phone" << endl;
			}
			continue;
		}
		if (command == "print"){
			...
			if(it != nameSearch.end()){
				...
				cout << "There is no abonent with this name" << endl;
			}
			continue;
		}
